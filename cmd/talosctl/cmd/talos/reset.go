...
var resetCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		if resetCmdFlags.debug {
			resetCmdFlags.noWait = false
		}

		resetRequest := buildResetRequest()

		if resetCmdFlags.noWait {
...
func init() {
	...
	resetCmd.Flags().BoolVar(&resetCmdFlags.noWait, "no-wait", false, "do not wait for the operation to complete, return immediately. always set to false when --debug is set")
